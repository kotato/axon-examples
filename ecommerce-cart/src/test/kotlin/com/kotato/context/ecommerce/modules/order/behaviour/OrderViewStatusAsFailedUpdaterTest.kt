...
class OrderViewStatusAsFailedUpdaterTest {

    private val repository: OrderViewRepository = mockObject()
    private val service = OrderViewStatusAsFailedUpdater(repository)
    private val handler = UpdateOrderViewStatusOnOrderFailedEventHandler(service)

    @Test
    fun `it should update order view status to failed`() {
        val view = OrderViewStub.random()
        val expected = view.copy(status = OrderStatus.FAILED)

        shouldSearchOrderView(view.id, view)
        shouldSaveOrderView(expected)

        handler.on(OrderFailedEventStub.random(aggregateId = view.id.asString()))
    }

    @Test
    fun `it should throw exception when trying to update order view status to failed`() {
        val id = OrderIdStub.random()

        shouldSearchOrderView(id, null)

        assertFailsWith<OrderViewNotFoundException> {
            handler.on(OrderFailedEventStub.random(aggregateId = id.asString()))
        }
    }
